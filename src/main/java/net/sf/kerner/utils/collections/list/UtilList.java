...
public class UtilList {
    ...
    public static <L> List<L> cutToSize(final List<L> list, final int size) {
	if (list.size() <= size) {
	    return list;
	}
     ...
     * </p>
     * Elements, that have same position in both lists and are equal to each
     * other are filtered out.
     * </p>
     * {@code null equals null}. {@code null} is kept in list only, if at this
     * position in both list elemnt is {@code null} or other list does not have
     * this position.
     * </p>
     * Resulting list size is always equal to size of longer list.
     *
     * @param <C>
     * @param c1
     *            first list
